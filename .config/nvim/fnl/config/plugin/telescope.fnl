...
(vim.keymap.set :n "<C-/>" "<cmd>Telescope current_buffer_fuzzy_find<cr>" {:desc "fuzzy find in buffer"})


; helpers to edit files I often want to edit
(vim.keymap.set :n :<leader>en #(tb.find_files {:cwd "~/.config/nvim" :path_display ["smart"]}) {:desc "Edit neovim config"})
...
(vim.keymap.set :n :<leader>ep #(tb.find_files {:cwd "~/.local/share/nvim/site/pack/packer/start" :path_display ["smart"] }) {:desc "Search plugin files"})
