...
(ls.config.set_config {:history true :update_events "TextChanged,TextChangedI"})

(vim.keymap.set :n :<leader>es loaders.edit_snippet_files
                {:desc "Edit external snippet files"})

(vim.keymap.set :n :<leader><leader>s edit-snippets
                {:desc "Edit fnl-defined snippets"})
...
(let [t ls.text_node
      ...
      s ls.snippet]
  (ls.add_snippets :fennel [(s {:trig :nmod
                                :name :module
                                :dscr "Add the preamble for a new Neovim module"}
                               [(t "(module ")
  ...
  (ls.add_snippets :javascript
                   [(s {:trig :fun
                        :name :function
                        :dscr "Good old fashioned function"}
